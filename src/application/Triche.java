...
public class Triche
{
	...
	private static void tricheMonter(List colonneCartes)
	{
		...
		for (int x = 0; x < colonneCartes.size(); x++) {
			if (x < colonneCartes.size() - 1) {
				pValeur = ((Carte) colonneCartes.get(posMax)).getValeur().getValeur();
				sValeur = ((Carte) colonneCartes.get(x + 1)).getValeur().getValeur();

				if ((sValeur == 1 ? 14 : sValeur) >= (pValeur == 1 ? 14 : pValeur)) {
					posMax = x + 1;
				}
			}
		}
		...
		for (int x = 0; x < colonneCartes.size(); x++) {
			if (cTrouve == null) {
				if (x != posMax) {
					p.empiler(colonneCartes.get(x));
				} else {
					cTrouve = (Carte) colonneCartes.get(x);
				}
			} else {
				p.empiler(colonneCartes.get(x));
			}
		...
		while (!p1.isEmpty()) {
			colonneCartes.add(p1.depiler());
		}

		colonneCartes.add(cTrouve);
	}
