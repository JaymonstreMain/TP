 ...
 *
 * @author Jérémy Marceau et François Marchand
 */
public class AcesUpSolitaire extends JFrame {
    ...
    public void gestionDeplacerListe(int indexColonne) {
        if (!colonneCartes[indexColonne].isEmpty() && colonneCartes[indexColonne].size() > 1) {
            Carte cCourante = (Carte) colonneCartes[indexColonne].get(0);

            for (int x = 0; x < AcesUpSolitaire.NBR_COLONNES_DE_CARTES; x++) {
                if (x != indexColonne) {
                    if (colonneCartes[x].isEmpty()) {
                        colonneCartes[x].add(cCourante);
                        dessinerListeCartes(x);
    ...
    public void gestionEnleverListe(int pNoListe) {
        if (!colonneCartes[pNoListe].isEmpty()) {
            Carte cCourante = (Carte) colonneCartes[pNoListe].get(0);
            Carte cBoucle;

            for (int x = 0; x < AcesUpSolitaire.NBR_COLONNES_DE_CARTES; x++) {
                if (x != pNoListe) {
                    if (!colonneCartes[x].isEmpty()) {
                        cBoucle = (Carte) colonneCartes[x].get(0);
                        if (cCourante.getSorte().toString().equals(cBoucle.getSorte().toString())) {
                            if ((cCourante.getValeur().getValeur() == 1 ? 14 : cCourante.getValeur().getValeur()) < (cBoucle.getValeur().getValeur() == 1 ? 14 : cBoucle.getValeur().getValeur())) {
        ...
        if (pioche.isEmpty()) {
            for (int i = 0; i < AcesUpSolitaire.NBR_COLONNES_DE_CARTES; i++) {
                if (colonneCartes[i].size() == 1) {
                    cBoucle = (Carte) colonneCartes[i].get(0);
                    if (cBoucle.getValeur().getValeur() == ValeurCartes.V_AS.getValeur()) {
                        cptAs++;
                    }
                }
            }
    ...
    public void enregistrerInfoPartie(File pCible) throws IOException {
        FileOutputStream monFichier = new FileOutputStream(pCible);
        ObjectOutputStream fich = new ObjectOutputStream(monFichier);
        fich.writeObject(colonneCartes);
        fich.writeObject(pioche);
        fich.flush();
        monFichier.close();
    }
    ...
    public void lireInfoPartie(File pSource)
        ...
        FileInputStream monFichier = new FileInputStream(pSource);
        ObjectInputStream fich = new ObjectInputStream(monFichier);
        // Il faut faire un cast
        colonneCartes = (List[]) fich.readObject();
        pioche = (Pioche) fich.readObject();
        monFichier.close();
